...
function FriendListItem({ ID, avatar, fullName }: IProps) {
  ...
  return (
    <div>
      <div className="flex space-x-2 border-2 p-2 mt-3 hover:cursor-pointer hover:border-gray-400 justify-items-center space-x-[800px]">
        <div className="flex space-x-2 hover:cursor-pointer">
          <img src={avatar} alt="" className="rounded-full w-8 h-8" />
          <p>{fullName}</p>
        </div>
        <div className="fixed" >
          <CircleEllipsisIcon onClick={toggleModal} />
